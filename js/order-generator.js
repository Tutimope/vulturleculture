...
let darkmodeState;
const Settings = {'MAIN_TABLE_WIDTH': 3,'COMBO_TABLE_WIDTH': 2,'DEFAULT_MAX_CAP': 10000}

const Menu = {
	"Gem/Relic/Jewelry Purchases:": {header: true},
	"LQ Citrine": {price: 300, items: ["LQ Citrine"]},
	"MQ Citrine": {price: 400, items: ["MQ Citrine"]},
	"HQ Citrine": {price: 500, items: ["HQ Citrine"]},
	"LQ Jade": {price: 50, items: ["LQ Jade"]},
	"MQ Jade": {price: 100, items: ["MQ Jade"]},
	"HQ Jade": {price: 150, items: ["HQ Jade"]}, 
    "LQ Aquamarine": {price: 100, items: ["LQ Aquamarine"]},
	"MQ Aquamarine": {price: 150, items: ["MQ Aquamarine"]},
	"HQ Aquamarine": {price: 200, items: ["HQ Aquamarine"]},
	"LQ Sapphire": {price: 200, items: ["LQ Sapphire"]},
	"MQ Sapphire": {price: 300, items: ["MQ Sapphire"]},
	"HQ Sapphire": {price: 400, items: ["HQ Sapphire"]}, 
    "LQ Ruby": {price: 200, items: ["LQ Ruby"]},
	"MQ Ruby": {price: 300, items: ["MQ Ruby"]},
	"HQ Ruby": {price: 400, items: ["HQ Ruby"]}, 
    "LQ Diamond": {price: 300, items: ["LQ Diamond"]},
	"MQ Diamond": {price: 400, items: ["MQ Diamond"]},
	"HQ Diamond": {price: 500, items: ["HQ Diamond"]},
	"LQ Tanzanite": {price: 300, items: ["LQ Tanzanite"]},
	"MQ Tanzanite": {price: 350, items: ["MQ Tanzanite"]},
	"HQ Tanzanite": {price: 400, items: ["HQ Tanzanite"]}, 
	"Unidentified Relic": {price: 500, items: ["Relic"]},
	"Jewelry Part": {price: 150, items: ["Jewelry Part"]},  
  
	"Ring/Relic Sales:": {header: true},
	"Jade Ring": {price: 7500, items: ["Jade Ring"]},
	"Citrine Ring": {price: 10000, items: ["Citrine Ring"]},
	"Aquamarine Ring": {price: 5000, items: ["Aquamarine Ring"]},
	"Sapphire Ring": {price: 7500, items: ["Sapphire Ring"]},
	"Relic Repair": {price: 10000, items: ["Relic Repair"]},
	"Relic Appraisal": {price: 15000, items: ["Relic Appraisal"]},
	"Gem Sales": {price: 1000, items: ["Gem Sales"]},
};

const INDIVIDUAL_ITEMS = [
	"LQ Citrine", "MQ Citrine", "HQ Citrine", "LQ Jade",
	"MQ Jade", "HQ Jade", "LQ Aquamarine", "MQ Aquamarine",
	"HQ Aquamarine", "LQ Sapphire", "MQ Sapphire", "HQ Sapphire",
	"LQ Ruby", "MQ Ruby", "HQ Ruby", "LQ Diamond", "MQ Diamond",
	"HQ Diamond", "LQ Tanzanite", "MQ Tanzanite", "HQ Tanzanite",
	"Jade Ring", "Citrine Ring", "Aquamarine Ring", "Sapphire Ring",
	"Relic", "Jewelry Part", "Relic Repair", "Relic Appraisal", "Gem Sales"
];

const buttons = {
	"buffer": {html: '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp'},
	"new_order": {html: '<div class="btn" onclick="newOrder();" title="Clear current order"><i class="fa fa-refresh" aria-hidden="true"></i> New Order</div>'},
	"set_combos": {html: '<div class="btn" onclick="toggleCombos()" title="Select current active combos"><i class="fa fa-cog" aria-hidden="true"></i> Combo Settings</div>'},
	"save": {html: '<div class="btn" onclick="updateSelected()" title="Save selected combos"><i class="fa fa-floppy-o" aria-hidden="true"></i> Save Combination Items</div>'},
	"deselect_combos": {html: '<div class="btn" onclick="deselectCombos()" title="Deselect all combos"><i class="fa fa-times-circle-o" aria-hidden="true"></i> Deselect All</div>'}
};

const discounts = {
	"half_off": {percent: .50, desc: '50% Discount (PD)'},
	"twentyfive_off": {percent: .25, desc: '25% Discount (EMS)'},
};
...
function formatItems(items) {
	...
	INDIVIDUAL_ITEMS.forEach(item => {
		...
		let imageIcon = `<img src="css/images/${imageName}.png" title="${occ}x ${item}" width="30" height="30"> `
...
function getEmptyOrder() {
	let buffer = [];
	buffer.push('<img src="css/images/VLC.png" width="25%">');
	buffer.push("");
...
function report() {
	let buffer = [];
	buffer.push('<img src="css/images/VLC.png" width="15%">');
	buffer.push("");
...
function getIcon(item) {
	...
	if (Menu[item].emoji) {
		...
		icon = `<img src="css/images/${fileName}" width="20" height="20">`;
	}
	return icon;
}
